...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 10.0 && time <= 13.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 16.0 && time <= 19.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if (time >= 21.0 && time <= 23.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >= 41.5 && time <= 44.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 65.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
